...
**keypoint 매핑**: AIHub 163 keypoint는 COCO-17이 아니라 16개 점이다. 공식 가이드
문서가 없어 실제 이미지에 점을 찍어 눈으로 확인하는 방식으로 best-effort 매핑을
추론했다(`docs/keypoint_mapping.md`, 2026-07-06). 이 스크립트 자체는 원본 16-포인트
순서를 그대로 보존해서 저장하며(`joint_0`..`joint_15`), 그래프(인접행렬) 설정은
HD-GCN 통합 시점에 `docs/keypoint_mapping.md`의 추론 매핑을 사용한다 — 단 2프레임만
검증된 상태이므로 확정으로 간주하지 말 것.
"""
