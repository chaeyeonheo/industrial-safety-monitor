...
def draw_frame(img, result) -> None:
    for status in result.ppe_statuses:
        ...
        box_color = COLOR_ALERT if status.missing_items else COLOR_NEUTRAL
        cv2.rectangle(img, (x1, y1), (x2, y2), box_color, 2)

        label = f"ID {status.track_id}"
        (tw, th), _ = cv2.getTextSize(label, FONT, 0.55, 1)
        draw_rounded_panel(img, x1, y1 - th - 10, x1 + tw + 12, y1, COLOR_PANEL_BG, alpha=0.75)
        cv2.putText(img, label, (x1 + 6, y1 - 6), FONT, 0.55, (240, 240, 240), 1, cv2.LINE_AA)

        if status.missing_items:
            text = "미착용: " + ", ".join(status.missing_items)
            (tw, th), _ = cv2.getTextSize(text, FONT, 0.5, 1)
            draw_rounded_panel(img, x1, y2 + 4, x1 + tw + 12, y2 + th + 16, COLOR_PANEL_BG, alpha=0.75)
            cv2.putText(img, text, (x1 + 6, y2 + th + 8), FONT, 0.5, COLOR_WARNING, 1, cv2.LINE_AA)

    if result.alarm_texts:
        h, w = img.shape[:2]
        panel_h = 28 + 24 * len(result.alarm_texts)
        draw_rounded_panel(img, 0, 0, w, panel_h, COLOR_PANEL_BG, alpha=0.80)
        cv2.putText(img, "SAFETY MONITOR", (14, 20), FONT, 0.55, (235, 235, 235), 1, cv2.LINE_AA)
        for i, text in enumerate(result.alarm_texts):
            color = COLOR_ALERT if "낙상" in text else COLOR_WARNING
            cv2.putText(img, text, (14, 46 + 24 * i), FONT, 0.55, color, 1, cv2.LINE_AA)
...
def main() -> None:
    ...
    saved_example = False
    total_alerts = 0
    for frame_idx, tracks in enumerate(pipeline.tracker.track_stream([str(p) for p in frame_paths])):
        img = cv2.imread(str(frame_paths[frame_idx]))
        result = pipeline.process_frame(frame_idx, tracks, img)
